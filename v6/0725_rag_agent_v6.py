...
- 0725_rag_agent_v6.py：Streamlit 畫面、檔案管理、對話與串流控制。
- rag_core_v6.py：管理文件、共同文字塊與 Chroma 向量資料庫。
- agent_core_v6.py：管理 Ollama 模型、Agent 與四個文件工具。
- hybrid_retriever_v6.py：合併 Chroma 向量搜尋與 BM25 關鍵字搜尋。
- phoenix_tracing_v6.py：記錄問答、檢索與模型執行狀態。

一般問答的完整流程：

輸入問題 → 組合近期對話 → 建立 Agent → Agent 選擇工具
→ 全庫或指定文件使用 Chroma＋BM25 混合檢索
→ Agent 依片段回答 → 顯示答案與來源
→ 將本輪結果存入 Session State

主題式文件清單維持「檔名比對＋向量搜尋」。如果 BM25 暫時不可用，
全庫與指定文件搜尋會自動退回純向量搜尋，不會阻擋問答。





2026-07-25 Agent v5.1
...
- 1.加入 Phoenix 本機追蹤
    記錄 Agent、工具、向量檢索與模型執行流程；
    Phoenix 無法連線時，主要問答功能仍可繼續使用。

- 2.建立混合式檢索
    結合 Chroma 向量搜尋與 BM25 關鍵字搜尋，使用 Ensemble Retriever 的 RRF 排名融合：
    向量搜尋權重：0.7
    BM25 關鍵字搜尋權重：0.3
    RRF 常數：60
...
- 4.同步管理 Chroma 與 BM25
    文件上傳、刪除或強制重建時會同步更新 BM25。
    BM25 無法使用時，問答會暫時退回純向量搜尋。

- 5.改善來源顯示
    一般對話不顯示文件來源；DOCX 不顯示不可靠的頁碼。

- 6.改善 LLM 輸出格式
    將 LLM 輸出的 <br>、<li>、<ul>、<ol> 等 HTML 標籤轉換成 Markdown 格式，避免畫面直接顯示 HTML 語法。
...
from agent_core_v6 import (
    get_llm,
    _content_to_text,
    create_pathology_agent,
    get_display_page_number,
    has_document_retrieval_sources,
    normalize_llm_answer,
    get_tool_display_name,
    emit_live_agent_event,
    format_agent_execution_event,
)
from phoenix_tracing_v6 import (
    finish_question_trace,
    get_phoenix_status,
    record_user_feedback,
    start_question_trace,
)
...
# 先嘗試連接本機 Phoenix。服務不存在或套件未安裝時只更新狀態，
# 不會影響一般使用者問答。
...
def render_source_documents(docs, heading="#### 🔍 Agent 檢索來源片段："):
    """將 Agent 本次取回的 LangChain Document 顯示成來源折疊區。

    每個 Document 的 metadata 會保留 source；只有 PDF 會顯示頁碼。DOCX 與 CSV 不顯示頁碼。
    """
    if not docs:
    ...
    for index, doc in enumerate(docs, start=1):
        ...
        if page_number is not None:
            header_text += f" (第 {page_number} 頁)"

        with st.expander(header_text):
            # 先 escape 文件內容，再以 pre-wrap 保留 DOCX 段落與換行；
            # 同時允許長檔名、英文與病理學名詞在窄畫面中正常換行。
            content = html.escape(str(doc.page_content or ""))
            st.markdown(
                '<div style="font-size: 0.9em; color: #505050; '
...
def file_management_center():
    ...
    兩個分頁分別負責：
        1. 上傳：檢查加密與同名檔，必要時轉檔，再加入 Chroma。
        2. 管理：刪除實體文件及其對應的 Chroma 文字塊。

    一批上傳或刪除完成後，系統會再統一重建一次 BM25，避免每處理
    一個檔案就重掃整個資料夾。

    管理功能具敏感性，因此只會由已登入的管理員按鈕開啟。
    """
    ...
    with tab_upload:
        ...
        if st.button("上傳並更新資料庫", icon="💾", use_container_width=True):
            if uploaded_files:
                ...
                with st.spinner("系統正在處理檔案，請稍候..."):
                    ...
                    for idx, file in enumerate(uploaded_files):
                        ...
                        try:
                            ...
                            if ext == '.doc':
                                # v6 只使用作業系統實際找到的執行檔，不再回退到
                                # Windows 固定路徑，確保 Linux 缺套件時能清楚回報。
                                soffice_path = (
                                ...
                                if not soffice_path:
                                    raise RuntimeError(
                                        "找不到 LibreOffice。Linux 請先安裝 "
                                        "libreoffice，或改上傳 DOCX 格式。"
                                    )
                            ...
                            # [狀況 C] PDF、DOCX、CSV 不需要轉檔，可直接加入 Chroma；
                            # 整批檔案處理完成後，再一起同步 BM25。
                            else:
                ...
                # 一批檔案處理完成後只重建一次 BM25，避免每個檔案重複掃描整個資料夾。
                if save_count > 0:
                    try:
                        rebuild_bm25_index(
                        ...
                        st.session_state["bm25_sync_warning"] = (
                            f"BM25 同步失敗，已保留舊索引；目前問答將暫時使用純向量搜尋。\n{error}"
                        )
...
def confirm_rebuild_dialog():
    """顯示二次確認視窗，確認後重建 Chroma 與 BM25。

    系統會先重新建立 Chroma，再用相同的文件文字塊建立 BM25。
    由於過程可能耗時，畫面會顯示五個階段的進度。取消或完成後，
    頁面會重新整理並關閉對話框。
    """
...
SYSTEM_SETTINGS_STATE_VERSION = "v6-defaults-20260729-01"
...
if "_phoenix_session_id" not in st.session_state:
    # 使用隨機值串起同一個 Streamlit 工作階段，不記錄帳號或病人識別資料。
    st.session_state._phoenix_session_id = uuid.uuid4().hex
...
with st.sidebar:
    ...
    if st.session_state.role == "user":
        ...
        bm25_status = get_bm25_index_status(
            processed_data_path=processed_data_path,
        )
        if bm25_status["available"]:
            st.success(
                "BM25 混合檢索已同步 "
                f"（{bm25_status['document_count']} 個 chunk）"
            )
        else:
            st.warning(
                "BM25 尚未可用，問答會暫時退回純向量搜尋。\n"
                f"原因：{bm25_status['reason']}"
            )
        ...
        st.divider()
        st.subheader("Phoenix 本機追蹤")
        # 重新讀取狀態可讓管理員在啟動 Phoenix 後，不必重開 Streamlit。
        phoenix_status = get_phoenix_status()
        if phoenix_status["available"]:
            st.success("Phoenix 已連線，將記錄本次 Agent 與 RAG 流程。")
        elif phoenix_status["enabled"]:
            st.warning(
                "Phoenix 未連線；問答功能仍可使用，但本次不會產生追蹤紀錄。"
            )
        else:
            st.info("Phoenix 追蹤目前已停用。")
        st.caption(
            f"專案：{phoenix_status['project_name']}  \n"
            f"位址：{phoenix_status['endpoint']}  \n"
            
        )
        if phoenix_status.get("detail"):
            st.caption(f"狀態說明：{phoenix_status['detail']}")
    ...
    if st.session_state.get("bm25_sync_warning"):
        st.warning(st.session_state["bm25_sync_warning"])
...
def render_answer_feedback(message, message_index):
    """顯示回答回饋按鈕，並將結果附加到 Phoenix 根 Span。

    Phoenix 未連線時仍會把選擇保存在本次 Session State，讓使用者操作
    不會失敗；畫面只說明尚未同步，不顯示任何內部位址或技術錯誤。
    """
    if message.get("role") != "assistant":
        return

    saved_feedback = message.get("user_feedback")
    if saved_feedback in {"positive", "negative"}:
        feedback_text = (
            "有幫助" if saved_feedback == "positive" else "需要改進"
        )
        sync_text = (
            "，已同步分析紀錄"
            if message.get("feedback_synced")
            else "，目前只保存在本次對話"
        )
        st.caption(f"您的回饋：{feedback_text}{sync_text}。")
        return

    execution_key = (
        message.get("execution_id")
        or f"feedback-message-{message_index}"
    )
    positive_column, negative_column, spacer_column = st.columns([1, 1, 5])
    positive_clicked = positive_column.button(
        "👍 有幫助",
        key=f"feedback_positive_{execution_key}",
    )
    negative_clicked = negative_column.button(
        "👎 需改進",
        key=f"feedback_negative_{execution_key}",
    )
    if not (positive_clicked or negative_clicked):
        return

    positive = bool(positive_clicked)
    feedback_name = "positive" if positive else "negative"
    synced, detail = record_user_feedback(
        message.get("phoenix_span_id", ""),
        positive=positive,
    )
    message["user_feedback"] = feedback_name
    message["feedback_synced"] = synced
    if synced:
        st.toast("感謝您的回饋，已加入系統分析紀錄。")
    else:
        st.warning(f"回饋已保存在本次對話，但尚未同步。{detail}")
    st.rerun()
...
for message_index, message in enumerate(st.session_state.messages):
    with st.chat_message(message["role"]):
        ...
        st.markdown(normalize_llm_answer(message["content"])) # 顯示對話文字內容
        ...
        if (
            ...
            and "docs" in message
            and has_document_retrieval_sources(
                docs=message["docs"],
                tool_calls=message.get("tool_calls", []),
                retrieval_records=message.get("retrieval_records", []),
            )
        ):
        ...
        render_answer_feedback(message, message_index)
...
if prompt_input := st.chat_input("請輸入關於病理科流程的問題..."):
    ...
    # 問答一開始就建立根 Trace，因此「知識庫沒有文件」等提早結束情境
    # 也能留下可觀察紀錄。Phoenix 不可用時會回傳空 handle。
    search_state = {
        "docs": [],
        "retrieval_records": [],
        "retrieval_runs": [],
    ...
    if isinstance(st.session_state.active_document, dict):
        active_document_name = str(
            st.session_state.active_document.get("display_name", "")
        )
    question_trace = start_question_trace(
        prompt_input,
        execution_id=execution_id,
        session_id=st.session_state._phoenix_session_id,
    ...
    with st.chat_message("assistant"):
        ...
        if not valid_files:
            ...
            st.session_state.messages.append({
                ...
                "docs": [],
                "execution_id": execution_id,
                "phoenix_trace_id": question_trace.trace_id,
                "phoenix_span_id": question_trace.span_id,
            })
            finish_question_trace(
                question_trace,
                answer=fallback_msg,
        ...
        try:
            ...
            # 寫入時的備援工具紀錄。
            detected_tool_calls = []
            # 以 LangChain tool_call_id 對應開始時間，計算每個工具的耗時。
                    ...
                    if node_name == "model":
                        ...
                        if tool_calls:
                            for tool_call in tool_calls:
                                ...
                                tool_call_started_at[tool_call_id] = (
                                    time.perf_counter()
                                )
                        ...
                        for completed_message in node_messages:
                            ...
                            # Phoenix 根 Trace 另存完整工具輸入／輸出、狀態與
                            # 耗時；內容來自已公開給 Agent 的 ToolMessage，
                            # 不包含模型未公開的內部推理。
                            matching_detected_call = next(
                                (
                                    tool_call
                            ...
                            tool_started_at = tool_call_started_at.get(
                                completed_tool_call_id
                            )
                            tool_elapsed_ms = (
                                round(
                            ...
                            search_state.setdefault(
                            ...
                            ).append(
                                {
                                    ...
                                    "arguments": matching_detected_call.get(
                                        "arguments",
                                        {},
                                    ),
                                    "output": completed_tool_content,
                                    "status": result_state,
                            ...
                            if should_display_directly and completed_tool_content:
                                ...
                                streamed_answer_parts = []
                                answer_placeholder.markdown(
                                    normalize_llm_answer(direct_tool_answer)
                                )
                                emit_live_agent_event(
                                    status_container=live_status,
                                    execution_log=execution_log,
            ...
            # Phoenix 優先使用模型事件中的原始參數與 call ID；若模型事件
            # 不完整，再退回工具本身保存的簡化紀錄。
            trace_tool_calls = (
                detected_tool_calls
            ...
            if direct_tool_answer:
                ...
                if direct_tool_notice:
                    st.info(direct_tool_notice)
            elif has_document_retrieval_sources(
            ...
            assistant_message = {
                ...
                "phoenix_span_id": question_trace.span_id,
                "tool_calls": completed_tool_calls,
                "retrieval_records": search_state.get(
                    "retrieval_records",
                    [],
                ),
            ...
            failed_tool_calls = (
                search_state.get("tool_calls", [])
                or locals().get("detected_tool_calls", [])
            )
            failed_trace_tool_calls = (
                locals().get("detected_tool_calls", [])
                or failed_tool_calls
            )
            finish_question_trace(
                question_trace,
                answer=error_msg,
                search_state=search_state,
                tool_calls=failed_trace_tool_calls,
                error=e,
            )
